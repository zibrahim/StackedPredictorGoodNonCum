...
from Models.Constraints import AttentionDecoder
from keras.callbacks import EarlyStopping, ModelCheckpoint
from keras.regularizers import l1

class LSTMAutoEncoder():
    def __init__(self, name, outcome, timesteps, n_features, custom_objects, saved_model = None):
        if saved_model == None:
            self.lstm_autoencoder = Sequential(name = name)
        # Encoder

            self.lstm_autoencoder.add(LSTM(100, activation='linear',
                                           activity_regularizer=l1(0.000001),
                                           input_shape=(timesteps, n_features), return_sequences=True))
            self.lstm_autoencoder.add(LSTM(100, activation='relu',
                                           input_shape=(timesteps, n_features), return_sequences=False))
            #self.lstm_autoencoder.add(Dropout(0.5))
            #self.lstm_autoencoder.add(LSTM(32, activation='relu', return_sequences=False))
            self.lstm_autoencoder.add(Dropout(0.5))
            self.lstm_autoencoder.add(RepeatVector(timesteps))
        # Decoder
            self.lstm_autoencoder.add(AttentionDecoder(100, n_features))
            ...
            self.lstm_autoencoder = load_model(saved_model, custom_objects = custom_objects)
    ...
    def fit(self, trainx,e, b,val_x,v):
        configs = json.load(open('Configuration.json', 'r'))
        autoencoder_models_path = configs['paths']['autoencoder_models_path']
        es = EarlyStopping(monitor='val_loss', mode='min',
                           verbose=1, patience=200, restore_best_weights=True)
        ...
        history = self.lstm_autoencoder.fit(trainx, trainx,epochs = e, batch_size = b,
                          validation_data = (val_x,val_x), verbose = v,
